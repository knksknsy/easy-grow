            ...
            if ((err = netconn_recv(client, &nb)) == ERR_OK) {
                ...
                if (!strncmp(data, "GET ", 4)) {
                    ...
                    if (!strncmp(ptr, "/submit", max_uri_len)){
						// übernächsten Abschnitt (SSID) auslesen
						ptr = strtok(NULL, delimiter);
                    	ptr = strtok(NULL, delimiter);
                    	char ssid[32];
                    	memcpy(ssid,ptr, 32);
                    	// übernaechsten Abschnitt (PW) auslesen
                    	ptr = strtok(NULL, delimiter);
                    	ptr = strtok(NULL, delimiter);
                    	char pwd[64];
                    	memcpy(pwd,ptr, 64);
                    ...
                    snprintf(buf, sizeof(buf), webpage,
                            available_aps[0], available_aps[1], available_aps[2], available_aps[3], available_aps[4],
                            xTaskGetTickCount() * portTICK_PERIOD_MS / 1000,
                            (int) heap_caps_get_free_size(MALLOC_CAP_8BIT));
                    netconn_write(client, buf, strlen(buf), NETCONN_COPY);
                }
...
void set_aps(wifi_ap_record_t aps[], uint16_t apCount) {
	for (int i = 0; i < 5; i++)
	{
		...
		if(i == 0 && apCount == 0) {
			memcpy(available_aps[i], "Kein Wlan gefunden!", sizeof(available_aps[i]));
		}
